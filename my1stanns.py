...
# 参数初始化
X = np.array([[2, 1], [-1, 1], [-1, -1], [1, -1]])  # 用于训练的坐标，对应的是I、II、III、IV象限
...
for j in range(10000):  # 这里设置了训练的循环次数为10000
    # ①前向传播
    H, fc_cache = affine_forward(X, W1, b1)  # 第一层前向传播（缓存一份用于反向传播）
    H = np.maximum(0, H)  # 激活（Relu函数）
